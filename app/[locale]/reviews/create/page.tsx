...
function CreateReviewContent() {
  ...
  // Fetch Categories and Book Details (if book_id exists)
  useEffect(() => {
        ...
        >
          <div className="space-y-8">
            ...
            >
              <div className="border-b border-gray-200 dark:border-gray-800 p-3 bg-gray-50/50 dark:bg-gray-900 flex items-center gap-2 flex-wrap">
                <button type="button" onClick={() => insertText("**", "**")} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400"><strong>B</strong></button>
                <button type="button" onClick={() => insertText("*", "*")} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400"><em>I</em></button>
                <button type="button" onClick={() => insertText("# ")} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400 font-bold">H1</button>
                <button type="button" onClick={() => insertText("- ")} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400"><IoList /></button>
                <button type="button" onClick={() => insertText("[", "](url)")} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400"><IoLink /></button>
                <button type="button" onClick={handleInsertImageInContent} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400"><IoImage /></button>
                <button type="button" onClick={() => insertText("`", "`")} className="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400"><IoCode /></button>
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-900 p-5 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800">
              ...
              >
                {isLoading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><IoSave className="w-5 h-5" />Terbitkan</>}
              </button>
            </div>
            ...
            >
              ...
              <div className="flex justify-between px-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} type="button" onClick={() => handleRating(star)} className={`p-2 rounded-xl transition-all duration-300 ${formData.rating >= star ? "text-yellow-400 bg-yellow-400/10 scale-110 shadow-lg shadow-yellow-400/20" : "text-gray-300 bg-gray-50 dark:bg-gray-800 hover:text-yellow-300"}`}><IoStar className="w-8 h-8" /></button>
                ))}
              </div>
            </motion.div>
            ...
            >
              <h3 className="font-bold text-gray-900 dark:text-white mb-2 flex items-center gap-2"><IoBook className="text-brand-500" /> Detail Buku</h3>
              <input type="text" name="book_title" value={formData.book_title} onChange={handleChange} className="w-full px-4 py-2 rounded-lg bg-gray-50 dark:bg-gray-800 border-none text-sm" placeholder="Judul Buku" />
              <input type="text" name="book_author" value={formData.book_author} onChange={handleChange} className="w-full px-4 py-2 rounded-lg bg-gray-50 dark:bg-gray-800 border-none text-sm" placeholder="Penulis" />
              <select name="category_id" value={formData.category_id} onChange={handleChange} className="w-full px-4 py-2 rounded-lg bg-gray-50 dark:bg-gray-800 border-none text-sm">
            ...
            <motion.div
              ...
              className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800"
            >
              <h3 className="font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2"><IoImage className="text-purple-500" /> Cover Buku</h3>
              <div onClick={() => fileInputRef.current?.click()} className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer transition-all ${dragActive ? "border-brand-500 bg-brand-50 dark:bg-brand-900/20" : "border-gray-300 dark:border-gray-700 hover:border-brand-400 hover:bg-gray-50 dark:hover:bg-gray-800/50"}`}>
                <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
                {formData.cover_url ? (
                  <div className="relative w-full aspect-[2/3] rounded-lg overflow-hidden shadow-md group">
                    <Image src={formData.cover_url} alt="Preview" fill className="object-cover" />
                  </div>
                ) : (
